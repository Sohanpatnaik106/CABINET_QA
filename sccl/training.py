...
class SCCLvTrainer(nn.Module):
    ...
    def train(self):
        ...
        self.model.train()
        for i in tqdm(np.arange(self.args.max_iter+1), position = 0, leave = True, total = self.args.max_iter+1):
            try:
    ...
    def evaluate_embedding(self, step):
        ...
        pred_labels = torch.tensor(kmeans.labels_.astype(np.int_))

        cluster_assignment_dict = {}
        idx = 0
        for i, batch in tqdm(enumerate(dataloader), position = 0, leave = True, total = len(dataloader)):
            for text in batch["text"]:
                cluster_assignment_dict[idx] = {}
                cluster_assignment_dict[idx]["question"] = text
                cluster_assignment_dict[idx]["cluster_label"] = pred_labels[idx].item()
                idx += 1
        
        print(f"Cluster assigned to {len(list(cluster_assignment_dict.keys()))} samples")
        with open(self.args.resPath + 'wikitq_cluster_assignment_{}.json'.format(step), "w") as f:
            json.dump(cluster_assignment_dict, f, indent = 4)
