...
class MPTForSequenceClassification(MPTPreTrainedModel):
    ...
    def __init__(self, config: MPTConfig, num_labels: int = 2):
        super().__init__(config, num_labels)
        if not config.tie_word_embeddings:
            raise ValueError('MPTForSequenceClassification only supports tied word embeddings')
    ...
    def forward(
        ...
        # else:
        #     assert, f"input_ids of the sequence cannot be None"
