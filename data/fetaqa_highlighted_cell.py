...
class FetaQAHighlightedCellsDataset(Dataset):

    def __init__(self, dataset, config, data_type):
        super(FetaQAHighlightedCellsDataset, self).__init__()
        ...
        if self.data_type == "train":
            with open("datasets/feta_qa_train_highlighted_cell.pkl", "rb") as f:
                self.highlighted_cells = pickle.load(f)

        elif self.data_type == "test":
            with open("datasets/feta_qa_test_highlighted_cell.pkl", "rb") as f:
                self.highlighted_cells = pickle.load(f)
    ...
    def _process_one_sample(self, data, idx = None):
        ...
        table_column_names = data["table_array"][0]
        table_content_values = data["table_array"][1:]
    ...
    def __getitem__(self, index) -> Any:
        ...
        if self.config.model.type == "encoder-decoder":
            ...
            if self.config.training.training_type == "description_generation" or self.config.training.training_type == "column_reasoning" \
                  or self.config.training.training_type == "table_question_answering" or self.config.training.training_type == "table_decomposition" or self.config.training.training_type == "descriptive_table_question_answering":
