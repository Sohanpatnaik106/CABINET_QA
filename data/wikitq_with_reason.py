...
class WikiTQWithReasonDataset(Dataset):

    def __init__(self, dataset, config, data_type):
        super(WikiTQWithReasonDataset, self).__init__()
    ...
    def __getitem__(self, index) -> Any:
        ...
        tokenized_reason = self._tokenize(self.reasons[index], max_length = self.config.tokenizer.reason_max_length)
        reason_labels = tokenized_reason["input_ids"][0].clone()
        
        if reason_labels[0] == self.tokenizer.convert_tokens_to_ids(self.tokenizer.special_tokens_map["bos_token"]):
            reason_labels[:-1] = reason_labels[1:].clone()
        else:
            tokenized_reason["input_ids"][0][1:] = tokenized_reason["input_ids"][0][:-1].clone()
            tokenized_reason["input_ids"][0][0] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.special_tokens_map["bos_token"])

        reason_labels[reason_labels == self.tokenizer.convert_tokens_to_ids(self.tokenizer.special_tokens_map["pad_token"])] = -100


        return tokenized_input["input_ids"].squeeze(), tokenized_input["attention_mask"].squeeze(), tokenized_input["token_type_ids"].squeeze(), \
                tokenized_output["input_ids"].squeeze(), labels, tokenized_reason["input_ids"].squeeze(), reason_labels
    


class WikiTQWithReasonAsInputDataset(Dataset):

    def __init__(self, dataset, config, data_type):
        super(WikiTQWithReasonAsInputDataset, self).__init__()
    ...
    def _process_one_sample(self, data, idx = None):
        ...
        text_input = f"question: {question} reason: {reason}"
        ...
        if self.config.training.training_type == "table_decomposition":
            ...
            return text_input, table, answer
