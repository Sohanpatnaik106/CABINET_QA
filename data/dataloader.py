    ...
    def __getitem__(self, index) -> Any:
        ...
        if self.config.model.type == "encoder-decoder":
            ...
            if self.config.training.training_type == "description_generation" or self.config.training.training_type == "column_reasoning":
                ...
                # Label is decode input ids with one token shifted left i.e., bos token is not required
...
"""
    Use ToTTO as the pre-training dataset. Given the table and the corresponding statement, ask the model to predic the highlighted cell values
        Train it in generative fashion using Encoder-Decoder model
        However
...
# TODO: Write the dataloader class for WikiSQL dataset?
# How exactly is it done? Train the model to generate SQL queries, then, execute it using some engine, check the official metric computation code