...
class WikiTQWithReasonAsInputBootstrapDataset(Dataset):

    def __init__(self, dataset, config, data_type):
        super(WikiTQWithReasonAsInputBootstrapDataset, self).__init__()
        ...
        if self.data_type == "train":
            with open("datasets/wiki_tq_reason_without_answer_flant5.pkl", "rb") as f:
            ...
            self.reasons = [""] * len(self.dataset["train"])

        self.text_input, self.table, self.text_output = self._process_dataset()

        indices_list = [i for i in range(len(self.text_input))]
        indices_list = random.sample(indices_list, len(indices_list))

        if self.data_type == "train":
            indices_list = indices_list[: int(0.7 * len(indices_list))]
            self.text_input = [self.text_input[x] for x in indices_list]
            self.table = [self.table[x] for x in indices_list]
            self.text_output = [self.text_output[x] for x in indices_list]
            self.reasons = [self.reasons[x] for x in indices_list]

            with open("datasets/bootstrap1_train_indices.pkl", "wb") as f:
            ...
            with open("datasets/bootstrap1_val_indices.pkl", "wb") as f:
                pickle.dump(indices_list, f)
    ...
    def _process_dataset(self):
        ...
        processed_data = []
        for i, data in tqdm(enumerate(self.dataset["train"]), position = 0, leave = True, total = len(self.dataset["train"])):
