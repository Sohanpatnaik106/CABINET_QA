...
class WikiSQLHighlightedCellsDataset(Dataset):

    def __init__(self, dataset, config, data_type):
        super(WikiSQLHighlightedCellsDataset, self).__init__()
        ...
        self.question_table_sep_token_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(" col"))[0]
        ...
        if self.data_type == "train":
            with open("datasets/wikisql_train_highlighted_cell.pkl", "rb") as f:
                self.highlighted_cells = pickle.load(f)

        elif self.data_type == "test":
            with open("datasets/wikisql_test_highlighted_cell.pkl", "rb") as f:
                self.highlighted_cells = pickle.load(f)
            
        else:
            self.highlighted_cells = torch.zeros((len(self.text_input), self.config.tokenizer.input_max_length), dtype = torch.int)
    ...
    def __getitem__(self, index) -> Any:
        ...
        if self.config.model.type == "encoder-decoder":
            ...
            if self.config.data.use_highlighted_cells:
                highlighted_cells = self.highlighted_cells[index]
                # question_table_sep_token_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(" col"))[0]
                index = tokenized_input["input_ids"][0].tolist().index(self.question_table_sep_token_id)
                # index = (tokenized_input["input_ids"][0] == question_table_sep_token_id).nonzero(as_tuple = True)[0][0]
                highlighted_cells[:index] = 1
                
                return tokenized_input["input_ids"].squeeze(), tokenized_input["attention_mask"].squeeze(), \
                        tokenized_input["token_type_ids"].squeeze(), tokenized_output["input_ids"].squeeze(), \
                            highlighted_cells, labels
...
class WikiSQLHighlightedCellsLogicalFormDataset(Dataset):

    def __init__(self, dataset, config, data_type):
        super(WikiSQLHighlightedCellsLogicalFormDataset, self).__init__()
        ...
        if self.data_type == "train":
            with open("datasets/wikisql_logic_train_highlighted_cell.pkl", "rb") as f:
                self.highlighted_cells = pickle.load(f)

        elif self.data_type == "test":
            with open("datasets/wikisql_logic_test_highlighted_cell.pkl", "rb") as f:
                self.highlighted_cells = pickle.load(f)
    ...
    def _process_one_sample(self, data, idx = None):
        ...
        answer = data["sql"]["human_readable"].lower()
        answer_list = None
    ...
    def __getitem__(self, index) -> Any:
        ...
        if self.config.model.type == "encoder-decoder":
            ...
            if self.config.training.training_type == "description_generation" or self.config.training.training_type == "column_reasoning" \
                  or self.config.training.training_type == "table_question_answering" or self.config.training.training_type == "table_decomposition" \
                    or self.config.training.training_type == "table_logic_generation":
                tokenized_output = self._tokenize(self.text_output[index], max_length = self.config.tokenizer.output_max_length)
                labels = tokenized_output["input_ids"][0].clone()
